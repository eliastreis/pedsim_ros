...
bool SceneServices::spawnPeds(pedsim_srvs::SpawnPeds::Request &request, pedsim_srvs::SpawnPeds::Response &response) {
  ...
  bool res = true; // spawnModelsInFlatland(flatland_models);
...
bool SceneServices::removeAllPeds(std_srvs::SetBool::Request &request,
  ...
  bool res = true; //removeModelsInFlatland(model_names);
  response.success = res;
  return res;
}
...
bool SceneServices::spawnInteractiveObstacles(pedsim_srvs::SpawnInteractiveObstacles::Request &request, pedsim_srvs::SpawnInteractiveObstacles::Response &response) {
  ...
  for (auto obstacle : request.obstacles) {
    ...
    std::vector<Obstacle*> new_walls = getWallsFromModel(obstacle, yaw);
  ...
  auto res = true; //spawnModelsInFlatland(new_models);
...
std::vector<Obstacle*> SceneServices::getWallsFromModel(pedsim_msgs::InteractiveObstacle obstacle, double yaw) {
  std::vector<Obstacle*> new_walls;
  YAML::Node model = YAML::LoadFile("/home/elias/catkin_ws/src/arena-rosnav-3D/simulator_setup/obstacles/turtlebot.model.yaml"); // ToDo change later
  auto bodies = model["bodies"];
  if (bodies.size() < 1) {
    ROS_ERROR("getWallsFromModel(): No body found in yaml node.");
    return std::vector<Obstacle*>();
  }

  auto footprints = bodies[0]["footprints"];
  if (footprints.size() < 1) {
    ROS_ERROR("getWallsFromModel(): No footprint found in body.");
    return std::vector<Obstacle*>();
  }
  ...
  if (type == NULL) {
    ROS_ERROR("getWallsFromModel(): No type found in footprint.");
    return std::vector<Obstacle*>();
  }
